...
#'head(deg)
#'@export
grab_deg_from_cuffdiff <- function(gene_exp.diff, out.name = NULL, class = c("mmu","gga","hsa"), criterion = c("p_value","q_value"), cutoff = 0.05){

  ############## get DEGs ##################
  deg <- gene_exp.diff[gene_exp.diff[,criterion] <= cutoff, ]
  ############## get gene symbol ##############
  mart <- eval(parse(text = paste0("mart_",class)))
